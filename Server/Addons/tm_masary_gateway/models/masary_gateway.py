...
from .masary_request import MASARYRequest

_logger = logging.getLogger(__name__)

class AcquirerMasaryChannel(models.Model):
    _inherit = 'payment.acquirer.channel'
    _order = "sequence"

    masary_login = fields.Char('Login', required_if_provider='masary', groups='base.group_user')                                # login:
    masary_password = fields.Char('Password', required_if_provider='masary', groups='base.group_user')                          # password:
    masary_terminalId = fields.Char('Terminal ID', required_if_provider='masary', groups='base.group_user')                     # terminalId: 123
    masary_account_number = fields.Char('Account Number', groups='base.group_user')                                             # accountNumber:
    masary_service_version = fields.Integer("Service Version")                                                                  # serviceVersion: 0

    # masary_acctCur = fields.Many2one("res.currency", string='Account Currency', required_if_provider='masary',
                                    # groups='base.group_user', default=lambda self: self.env.user.company_id.currency_id)      # acctCur: EGP

class AcquirerMasary(models.Model):
    _inherit = 'payment.acquirer'

    provider = fields.Selection(selection_add=[('masary', 'Masary')])

    masary_version =  fields.Integer("Interface Version", required_if_provider='masary', default=2)                             # version: 2
    masary_test_url = fields.Char("Test url", required_if_provider='masary',
                                 default='https://st-api01.e-masary.net/restgw/api')
    masary_prod_url = fields.Char("Production url", required_if_provider='masary',
                                 default='https://st-api01.e-masary.net/restgw/api')

    masary_channel_ids = fields.One2many('payment.acquirer.channel', 'acquirer_id', string='Masary Channels', copy=False)
    ...
    @api.onchange('masary_prod_url', 'masary_test_url')
    def _onchange_masary_url(self):
        self.server_url = self.masary_prod_url
        if self.environment == "test":
            self.server_url = self.masary_test_url
        # _logger.info(":[%s] ==> (%s, %s): %s" % (self.environment, self.masary_prod_url, self.masary_test_url, self.server_url))

    def get_masary_biller_details(self, masary_channel=None):
        superself = self.sudo()
        result = {}

        if not masary_channel and superself.masary_channel_ids:
            masary_channel = superself.masary_channel_ids[0]
        elif not masary_channel and not superself.masary_channel_ids:
            raise ValidationError(_('The fetch of masary biller details cannot be processed because the masary has not any chennel in confiquration!'))

        # # Tamayoz TODO: Remove temp condition below for handling masry service version updating
        # if masary_channel.masary_service_version == 0:
        # Production and Testing url
        endurl = superself.masary_prod_url
        if superself.environment == "test":
            endurl = superself.masary_test_url

        # _logger.info("endurl             >>>>>>>>>>>>>>>>>>>>> " + endurl)
        # _logger.info("login              >>>>>>>>>>>>>>>>>>>>> " + masary_channel.masary_login)
        # _logger.info("password           >>>>>>>>>>>>>>>>>>>>> " + masary_channel.masary_password)
        # _logger.info("version            >>>>>>>>>>>>>>>>>>>>> " + str(superself.masary_version))
        # _logger.info("terminalId         >>>>>>>>>>>>>>>>>>>>> " + masary_channel.masary_terminalId)
        srm = MASARYRequest(debug_logger=self.log_xml, endurl=endurl, env=self.env, login=masary_channel.masary_login, password=masary_channel.masary_password,
                            version=superself.masary_version, terminalId=masary_channel.masary_terminalId)

        custLangPref = 'AR'
        result_biller = srm.get_biller_details(custLangPref, data={"service_version": masary_channel.masary_service_version})
        if result_biller.get('billerData'):
            result['billerData_'+custLangPref] = result_biller['billerData']
            masary_channel.sudo().update({'masary_service_version': result_biller['billerData']['service_version']})
        else:
            result = result_biller

        return result

    def get_masary_service_details(self, biller_id, masary_channel=None):
        superself = self.sudo()
        result = {}
        ...
        # _logger.info("version            >>>>>>>>>>>>>>>>>>>>> " + str(superself.masary_version))
        # _logger.info("terminalId         >>>>>>>>>>>>>>>>>>>>> " + masary_channel.masary_terminalId)
        srm = MASARYRequest(debug_logger=self.log_xml, endurl=endurl, env=self.env,
                            login=masary_channel.masary_login, password=masary_channel.masary_password,
                            version=superself.masary_version, terminalId=masary_channel.masary_terminalId)
    ...
    def get_masary_service_input_parameters_details(self, service_id, masary_channel=None):
        superself = self.sudo()
        result = {}
        ...
        custLangPref = 'AR'
        result_service_input_parameters = srm.get_service_input_parameters_details(custLangPref, data={"service_id": service_id})
        if result_service_input_parameters.get('serviceInputParameterData'):
            result['serviceInputParameterData_' + custLangPref] = result_service_input_parameters['serviceInputParameterData']
        else:
            result = result_service_input_parameters

        return result

    def get_masary_service_output_parameters_details(self, service_id, masary_channel=None):
        superself = self.sudo()
        result = {}
        ...
        custLangPref = 'AR'
        result_service_output_parameters = srm.get_service_output_parameters_details(custLangPref, data={"service_id": service_id})
        if result_service_output_parameters.get('serviceOutputParameterData'):
            result['serviceOutputParameterData_' + custLangPref] = result_service_output_parameters['serviceOutputParameterData']
        else:
            result = result_service_output_parameters

        return result

    def get_masary_category_details(self, masary_channel=None):
        superself = self.sudo()
        result = {}
    ...
    def get_masary_category_service_details(self, masary_channel=None):
        superself = self.sudo()
        result = {}
        ...
        if not masary_channel and superself.masary_channel_ids:
            ...
            raise ValidationError(
                _('The fetch of masary category services details cannot be processed because the masary has not any chennel in confiquration!'))
    ...
    def auto_fetch_masary_biller_details(self):
        fetch_success = False
        ...
        for custLangPref in ['AR', 'EN']:
            ...
            if categoryData:
                for category in categoryData['category_list']:
                    _logger.info(" ====================================== Category Fetch Data Begin " + str(category['id']) + ": " + custLangPref + " =========================================")
                    ...
                    _logger.info(
                        " ====================================== Category Fetch Data End " + str(category['id']) + ": " + custLangPref + " =========================================")
            ...
            if billerData:
                for provider in billerData['provider_list']:
                    _logger.info(" ====================================== Biller Fetch Data Begin " + str(provider['id']) + ": " + custLangPref + " =========================================")
                    ...
                    if serviceData:
                        for service in serviceData['service_list']:
                            ...
                            result_service_input_parameters = masary.get_masary_service_input_parameters_details(serviceId)
                            serviceInputParameterData = result_service_input_parameters.get('serviceInputParameterData_' + custLangPref)
                            if serviceInputParameterData:
                                service.update({'input_parameter_list': serviceInputParameterData['input_parameter_list']})

                            # Get service output parameters list
                            result_service_output_parameters = masary.get_masary_service_output_parameters_details(serviceId)
                            serviceOutputParameterData = result_service_output_parameters.get('serviceOutputParameterData_' + custLangPref)
                            if serviceOutputParameterData:
                                service.update({'output_parameter_list': serviceOutputParameterData['output_parameter_list']})
                            ...
                            if serviceCategoryId:
                                ...
                                if service_categ_providerinfo:
                                    if not service_provider_providerinfo.product_categ_id.parent_id:
                                        service_provider_providerinfo.product_categ_id.sudo().update({'parent_id': service_categ_providerinfo.product_categ_id.id})
                                    elif service_provider_providerinfo.product_categ_id.parent_id.id != service_categ_providerinfo.product_categ_id.id\
                                            and len(service_provider_providerinfo.product_categ_id.parent_id.provider_ids) > 0:
                                        _logger.warning("Masary services for biller [%s: %s] have more than one category (%s: %s, %s:%s)"
                                                        % (billerId, billerName,
                                                           service_provider_providerinfo.product_categ_id.parent_id.provider_ids[0].product_categ_code,
                                                           service_provider_providerinfo.product_categ_id.parent_id.provider_ids[0].product_categ_name,
                                                           service_categ_providerinfo.product_categ_code, service_categ_providerinfo.product_categ_name))
                            else:
                                _logger.warning("Masary services [%s: %s] hasn't any category" % (serviceId, serviceName))
                                if not service_provider_providerinfo.product_categ_id.parent_id:
                                    service_provider_providerinfo.product_categ_id.sudo().update({'parent_id': self.env.ref("tm_base_gateway.product_category_mapping_services").id})
                    ...
                    _logger.info(
                        " ====================================== Biller Fetch Data End " + str(provider['id']) + ": " + custLangPref + " =========================================")
                fetch_success = True
    ...
    def get_masary_account_details(self, masary_channel=None):
        superself = self.sudo()
        result = {}
    ...
    def get_masary_bill_details(self, lang, billTypeCode, inputParameterList=None, masary_channel=None):
        superself = self.sudo()
        result = {}
        ...
        if not masary_channel and superself.masary_channel_ids:
            ...
            raise ValidationError(_('The fetch of masary bill details cannot be processed because the masary has not any chennel in confiquration!'))
        ...
        accountNumber = masary_channel.masary_account_number
        if not accountNumber:
            result_account = self.get_masary_account_details(masary_channel)
            if(result_account.get("accountData")):
                accountNumber = result_account["accountData"]["account_list"][0]["account_number"]
                masary_channel.sudo().update({'masary_account_number': accountNumber})

        data = {"service_version": masary_channel.masary_service_version, "account_number": accountNumber, "service_id": billTypeCode}
        ...
        if result_bill.get('billData'):
            ...
            '''
                ...
                # retry = True
                # while retry:
                    # # masary_channel.sudo().update({'masary_service_version': 0})
                    # self.auto_fetch_masary_biller_details()
                    # result = self.get_masary_bill_details(lang, billTypeCode, inputParameterList, masary_channel)
    ...
    def pay_masary_bill(self, lang, billTypeCode, amt, feeAmt,
                        ...
                        inputParameterList=None, masary_channel=None):
        superself = self.sudo()
        result = {}
        ...
        if not masary_channel and superself.masary_channel_ids:
            ...
            raise ValidationError(_('The pay of masary bill cannot be processed because the masary has not any chennel in confiquration!'))
        ...
        data = {"external_id": str(uuid.uuid1().time_low), "service_version": masary_channel.masary_service_version,
