    ...
    def button_confirm_bank(self):
        ...
        for statement in self:
            if statement.journal_type == 'cash' and statement.journal_id.provider_id:
                ...
                partner_ids = statement.env['res.partner'].sudo().search(
                    [('id', 'in', list(set([row[0] for row in rows])))])
                ...
                for partner_id in partner_ids:
                    ...
                    wallet_transaction_line_ids = wallet_transaction_line_sudo.search(
                    ...
                    for wallet_transaction_line_id in wallet_transaction_line_ids:
                        ...
                        if wallet_transaction_line_id.wallet_type == 'credit':
                            cashback_amount += float(
                                wallet_transaction_line_id.amount)  # Tamayoz TODO: Temp code for type custing until convert its type and amount field type in website.wallet.transaction to float
                    ...
                    customer_wallet_create.update(
                        {'amount': cashback_amount if wallet_type == 'credit' else cashback_amount * -1,
                         'wallet_balance_before': customer_wallet_balance,
                         'wallet_balance_after': customer_wallet_balance + cashback_amount,
                         # Whether cashback_amount positive or negative
                         'status': 'done'})

                    # Increase or Decrease the customer wallet balance with the net amount of provider payment
                    partner_id.update({
                                          'wallet_balance': partner_id.wallet_balance + cashback_amount})  # Whether cashback_amount positive or negative
