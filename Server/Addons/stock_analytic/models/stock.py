...
class StockMove(models.Model):
    ...
    def _prepare_account_move_line(self, qty, cost,
        ...
        if (not self.analytic_account_id and not self.analytic_tag_ids) or not res:
            return res
        ...
        for num in range(0, 2):
            if res[num][2]["account_id"] != self.product_id.\
                ...
                res[num][2].update({
                    'analytic_account_id': self.analytic_account_id.id, 'analytic_tag_ids': [(4, ati) for ati in self.analytic_tag_ids.ids],
                })
        return res
