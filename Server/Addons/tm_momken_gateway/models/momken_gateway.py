...
class AcquirerMomken(models.Model):
    ...
    def get_momken_biller_details(self, momken_channel=None):
        ...
        if not momken_channel and superself.momken_channel_ids:
            ...
            raise ValidationError(
                _('The fetch of momken biller details cannot be processed because the momken has not any chennel in confiquration!'))
        ...
        srm = MOMKENRequest(debug_logger=self.log_xml, endurl=endurl, env=self.env, login=momken_channel.momken_login,
                            password=momken_channel.momken_password,
    ...
    def get_momken_service_details(self, biller_id, momken_channel=None):
        ...
        if not momken_channel and superself.momken_channel_ids:
            ...
            raise ValidationError(
                _('The fetch of Momken service details cannot be processed because the Momken has not any chennel in confiquration!'))
        ...
        # _logger.info("login              >>>>>>>>>>>>>>>>>>>>> " + momken_channel.momken_login)
        # _logger.info("password           >>>>>>>>>>>>>>>>>>>>> " + momken_channel.momken_password)
        # _logger.info("version            >>>>>>>>>>>>>>>>>>>>> " + str(superself.momken_version))
        # _logger.info("terminalId         >>>>>>>>>>>>>>>>>>>>> " + momken_channel.momken_terminalId)
    ...
    def get_momken_service_input_parameters_details(self, service_id, momken_channel=None):
        ...
        if not momken_channel and superself.momken_channel_ids:
            ...
            raise ValidationError(
                _('The fetch of momken service input parameters details cannot be processed because the momken has not any chennel in confiquration!'))
    ...
    def get_momken_service_output_parameters_details(self, service_id, momken_channel=None):
        ...
        if not momken_channel and superself.momken_channel_ids:
            ...
            raise ValidationError(
                _('The fetch of momken service output parameters details cannot be processed because the momken has not any chennel in confiquration!'))
    ...
    def get_momken_category_details(self, momken_channel=None):
        ...
        if not momken_channel and superself.momken_channel_ids:
            ...
            raise ValidationError(
                _('The fetch of momken category details cannot be processed because the momken has not any chennel in confiquration!'))
    ...
    def auto_fetch_momken_biller_details(self):
        ...
        result_category = momken.get_momken_category_details()
        result_category_service = momken.get_momken_category_service_details()
        result_biller = momken.get_momken_biller_details()
        ...
        for custLangPref in ['AR', 'EN']:
            ...
            if billerData:
                for provider in billerData['provider_list']:
                    ...
                    # Get Service List
                    result_service = momken.get_momken_service_details(billerId)
                    ...
                    if serviceData:
                        for service in serviceData['service_list']:
                            ...
                            result_service_input_parameters = momken.get_momken_service_input_parameters_details(
                                serviceId)
                            ...
                            result_service_output_parameters = momken.get_momken_service_output_parameters_details(
                                serviceId)
                            ...
                            if serviceCategoryId:
                                ...
                                service_categ_providerinfo = self.env[
                                    ...
                                    ('product_categ_code', '=', serviceCategoryId), ('provider_id', '=', momken.id)
                                ])
                                if service_categ_providerinfo:
                                    if not service_provider_providerinfo.product_categ_id.parent_id:
                                        ...
                                        _logger.warning(
                                            "momken services for biller [%s: %s] have more than one category (%s: %s, %s:%s)"
                                            % (billerId, billerName,
    ...
    def get_momken_account_details(self, momken_channel=None):
        ...
        if not momken_channel and superself.momken_channel_ids:
            ...
            raise ValidationError(
                _('The fetch of momken account details cannot be processed because the momken has not any chennel in confiquration!'))
    ...
    def get_momken_bill_details(self, lang, billTypeCode, inputParameterList=None, momken_channel=None):
        superself = self.sudo()
        result = {}
        ...
        if not momken_channel and superself.momken_channel_ids:
            ...
            raise ValidationError(
                _('The fetch of momken bill details cannot be processed because the momken has not any chennel in confiquration!'))
        ...
        if result_bill.get('billData'):
            ...
            '''
                ...
                # while retry:
                    ...
                    # self.auto_fetch_momken_biller_details()
                    # result = self.get_momken_bill_details(lang, billTypeCode, inputParameterList, momken_channel)
