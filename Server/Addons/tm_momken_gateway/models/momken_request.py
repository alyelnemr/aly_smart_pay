...
class MOMKENRequest():
        ...
        try:
            ...
            if not provider_res['success']:
                _logger.error("MS Response ERROR: [" +
                              str(provider_res['error_code']) + "]: " + (
                                          provider_res.get('description') or provider_res.get('error_text')))
                return self.get_error_message(provider_res['error_code'],
                                              (provider_res.get('description') or provider_res.get('error_text')))
    ...
    def get_service_details(self, custLangPref, data):  # "data": {"provider_id": provider['id']}
        ...
        try:
            ...
            if not service_res['success']:
                _logger.error("MS Response ERROR: [" +
                              str(service_res['error_code']) + "]: " + (
                                          service_res.get('description') or service_res.get('error_text')))
                return self.get_error_message(service_res['error_code'],
                                              (service_res.get('description') or service_res.get('error_text')))
    ...
    def get_service_input_parameters_details(self, custLangPref, data):  # "data": {"service_id": service['id']}
        ...
        try:
            ...
            if not service_input_parameters_res['success']:
                _logger.error("MS Response ERROR: [" +
                              str(service_input_parameters_res['error_code']) + "]: " + (
                                          service_input_parameters_res.get(
                                              'description') or service_input_parameters_res.get('error_text')))
                return self.get_error_message(service_input_parameters_res['error_code'], (
                            service_input_parameters_res.get('description') or service_input_parameters_res.get(
                        'error_text')))
    ...
    def get_service_output_parameters_details(self, custLangPref, data):  # "data": {"service_id": service['id']}
        ...
        try:
            ...
            if not service_output_parameters_res['success']:
                _logger.error("MS Response ERROR: [" +
                              str(service_output_parameters_res['error_code']) + "]: " + (
                                          service_output_parameters_res.get(
                                              'description') or service_output_parameters_res.get('error_text')))
                return self.get_error_message(service_output_parameters_res['error_code'], (
                            service_output_parameters_res.get('description') or service_output_parameters_res.get(
                        'error_text')))
    ...
    def get_category_details(self, custLangPref, data):  # "data": {}
        ...
        try:
            ...
            if not category_res['success']:
                _logger.error("MS Response ERROR: [" +
                              str(category_res['error_code']) + "]: " + (
                                          category_res.get('description') or category_res.get('error_text')))
                return self.get_error_message(category_res['error_code'],
                                              (category_res.get('description') or category_res.get('error_text')))
    ...
    def get_category_service_details(self, custLangPref, data):  # "data": {}
        ...
        try:
            ...
            if not category_service_res['success']:
                _logger.error("MS Response ERROR: [" +
                              str(category_service_res['error_code']) + "]: " + (
                                          category_service_res.get('description') or category_service_res.get(
                                      'error_text')))
                return self.get_error_message(category_service_res['error_code'], (
                            category_service_res.get('description') or category_service_res.get('error_text')))
    ...
    def get_account_details(self, custLangPref, data):  # "data": {}
        ...
        try:
            ...
            if not account_res['success']:
                _logger.error("MS Response ERROR: [" +
                              str(account_res['error_code']) + "]: " + (
                                          account_res.get('description') or account_res.get('error_text')))
                return self.get_error_message(account_res['error_code'],
                                              (account_res.get('description') or account_res.get('error_text')))
    ...
    def get_bill_details(self, custLangPref, data):  # "data": {"service_version": 0, "account_number": "",
        ...
        try:
            ...
            if not bill_res['success']:
                _logger.error("MS Response ERROR: [" +
                              str(bill_res['error_code']) + "]: " + (
                                          bill_res.get('description') or bill_res.get('error_text')))
                return self.get_error_message(bill_res['error_code'],
                                              (bill_res.get('description') or bill_res.get('error_text')))
    ...
    def correlation_bill(self, custLangPref,
        ...
        try:
            ...
            momkenResponse = client.service.process(masaryType)
            # _logger.info("After Calling Masary Pay Bill")
            # _logger.info("PmtAddRq MasaryType Response: " + str(momkenResponse))
            ...
            # self._buildResponse(momkenResponse)
