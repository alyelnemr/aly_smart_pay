...
class LogPlugin(MessagePlugin):
    ...
    def sending(self, context):
        self.debug_logger(context.envelope, 'bee_request')

    def received(self, context):
        self.debug_logger(context.reply, 'bee_response')
...
class BeeRequest:
    ...
    def get_translated_message(self, msg):
        ...
        translated_msg = self.env['ir.translation'].sudo().search([
            ('type', '=', 'code'),
            ('name', '=', 'addons/tm_bee_gateway/models/bee_request.py'),
            ('source', '=', msg),
    ...
    def get_error_message(self, error_code, description):
        result = {}
        msg, is_translated = self.get_translated_message(BEE_ERROR_MAP.get(str(error_code)))
    ...
    def _buildResponse(beeResponse):
        _logger.info("Success: " + beeResponse['success'])
        if beeResponse['success']:
            _logger.info("Action: " + beeResponse['action'])
            _logger.info("Language: " + beeResponse['language'])
            _logger.info("Version: " + beeResponse['version'])
            _logger.info("Date: " + beeResponse['data'])
        else:
            _logger.info("Error Code: " + beeResponse['error_code'])
            _logger.info("Description: " + beeResponse['description'])
    ...
    def get_service_details(self, custLangPref, data):  # "data": {"provider_id": provider['id']}
        ...
        try:
            # Get All of services data from Bee
            ...
            if (authenticate_response.get('Code') and authenticate_response.get('Code') != '200') or (
                ...
                err_msg = "Bee Response ERROR: [" + str(authenticate_response.get('Message]')) + "]"
                _logger.error(err_msg)
                return err_msg
            bee = self.env['payment.acquirer'].sudo().search([("provider", "=", "bee")])
            bee_channel = bee.bee_channel_ids[0] if bee.bee_channel_ids else None
            if authenticate_response.get('version'):
                bee.bee_version = authenticate_response.get('version')
            if bee_channel and authenticate_response.get('serviceListVersion'):
                bee_channel.bee_service_version = authenticate_response.get('serviceListVersion')

            access_token = authenticate_response.get('token') if authenticate_response.get('token') else ''

            service_url = self.endurl + 'categories/' + str(bee_channel.bee_service_version)
            headers = {
                'content-type': 'application/json;charset=UTF-8',
